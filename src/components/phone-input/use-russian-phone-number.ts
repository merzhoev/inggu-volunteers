...
export function useRussianPhoneNumber({ onChange: onChangeArgument, value }: Options) {
  ...
  const [input, setInput] = useState({
    value: value ? fillMaskWithClearValue(value) : russianPhoneMask,
  });
  ...
  useEffect(
    () => onChangeArgument?.({ dirty: input.value, clear: getClearValue(input.value) }),
    [onChangeArgument, input.value],
  );
